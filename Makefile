...
#
# WINDOWS USERS:
#   - Requires GNU Make (available via MSYS2 or Chocolatey: choco install make)
...
# Windows detection: check OS environment variable (Windows) or ComSpec
OS_NAME := $(OS)
...
tests/test_json_helpers$(TEST_EXE_EXT): tests/test_json_helpers.cpp
ifeq ($(OS_NAME),Windows_NT)
	cl /std:c++17 /EHsc /Fe:tests\test_json_helpers.exe tests\test_json_helpers.cpp
else
	c++ -std=c++17 -o tests/test_json_helpers tests/test_json_helpers.cpp
endif

test-unit: tests/test_json_helpers$(TEST_EXE_EXT)
ifeq ($(OS_NAME),Windows_NT)
	tests\test_json_helpers$(TEST_EXE_EXT)
else
...
test-integration:
ifeq ($(OS_NAME),Windows_NT)
	python tests/test_server.py --port $(TEST_PORT)
else
...
ifeq ($(OS_NAME),Windows_NT)
	...
	if exist tests\test_json_helpers.exe del /q tests\test_json_helpers.exe
else
